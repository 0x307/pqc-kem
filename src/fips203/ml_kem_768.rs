...
impl zeroize::ZeroizeOnDrop for MlKem768Keypair {}
...
impl MlKem768Keypair {
    ...
    pub fn secret_key(&self) -> KemSecretKey {
        self.try_secret_key()
            .expect("ML-KEM-768 DecapsulationKey constructed via from_seed always has a recoverable seed")
    }
...
/// storage** — use [`MlKem768Keypair::generate`], [`MlKem768Keypair::encapsulate`],
/// and [`MlKem768Keypair::from_secret_key_bytes`] there instead.
#[cfg(feature = "kat")]
impl MlKem768Keypair {
    ...
    /// [`MlKem768Keypair::encapsulate`] already calls internally with an
    ...
    /// breaks ML-KEM's security guarantees — [`MlKem768Keypair::encapsulate`]
    ...
    /// "expanded" decapsulation-key wire format (2400 bytes for
    /// ML-KEM-768; see `ml_kem::ExpandedKeyEncoding`, deprecated since
    /// `ml-kem` 0.3.0 in favor of the 64-byte seed form). This crate's own
    /// public API never produces this format —
    /// [`MlKem768Keypair::secret_key`] / [`MlKem768Keypair::from_secret_key_bytes`]
    ...
    pub fn from_expanded_decapsulation_key_bytes(bytes: &[u8]) -> KemResult<Self> {
        let expanded: ml_kem::ExpandedDecapsulationKey<MlKem768> = bytes.try_into()
            .map_err(|_| KemError::InvalidKey(
                format!("ML-KEM-768 expanded decapsulation key must be 2400 bytes, got {}", bytes.len())
            ))?;

        #[allow(deprecated)]
        let dk = <DecapsulationKey<MlKem768> as ml_kem::ExpandedKeyEncoding>::from_expanded_bytes(&expanded)
            .map_err(|_| KemError::InvalidKey("ML-KEM-768 expanded decapsulation key failed validation".into()))?;
    ...
    /// [`MlKem768Keypair::from_expanded_decapsulation_key_bytes`]), so KAT
    ...
    pub fn to_expanded_decapsulation_key_bytes(&self) -> alloc::vec::Vec<u8> {
        #[allow(deprecated)]
        let expanded = <DecapsulationKey<MlKem768> as ml_kem::ExpandedKeyEncoding>::to_expanded_bytes(&self.decapsulation_key);
