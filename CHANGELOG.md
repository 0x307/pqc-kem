...
modules/features); see "Migration from 0.2.x" below.
...
- **`tests/kat_ml_kem.rs`** — for each of ML-KEM-512/768/1024: keyGen KAT (seed → `ek`/expanded
  ...
  a panic. Entirely `#[cfg(feature = "kat")]` — compiles to zero tests (a no-op) when the
...
- **`tests/kat_x25519.rs`** — RFC 7748 §5.2 scalar-mult and iterated vectors (1,000,000-iteration
  ...
  Alice/Bob worked example, via `fips203::hybrid::x25519_kat`. Also `#[cfg(feature = "kat")]`.
